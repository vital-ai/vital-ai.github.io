...
var eburl = 'http://' + hostPort + '/eventbus';
...
function mailing_signup(email, successCallback, errorCallback) {
	...
	eb.send('modernist.mailing.signup', {email: email}, function(result) {
		
		console.log('modernist.mailing.signup result: ', result)
...
$(function(){
  ...
  function fadeOutError() {
    error.fadeOut(1000, function(){
      ...
      $('.signup-els').animate({opacity: 1.0}, 1000, function(){
        inputEmail.focus();
      });
    });
  }
  ...
  function fadeOutSuccess() {
	...
	function onSignupTriggered() {
		...
		mailing_signup(email, function(msg){
			...
			$('.signup-els').animate({opacity: 0.0}, 1000, function(){
			   inputEmail.val('');
         els.removeAttr('disabled');
